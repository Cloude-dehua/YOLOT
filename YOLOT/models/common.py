...
#     def __init__(self, c1, c2, num_heads, num_layers, shortcut=True, g=1, e=0.5):
#         super().__init__()
#         self.conv = None
#         if c1 != c2:
#             self.conv = nn.Conv2d(c1, c2, kernel_size=1)  # 添加一个1x1卷积层
#         self.linear = nn.Linear(c2, c2)  # 可学习的位置编码
#         self.tr = nn.Sequential(*(TransformerLayer(c2, num_heads) for _ in range(num_layers)))
#         self.c2 = c2
#
#         self.parallel_bottleneck_block = Bottleneck(c2, c2, shortcut, g, e)  # 并行的bottleneck模块
#
#     def forward(self, x):
...
#
#         transformer_output = self.tr(p + self.linear(p))
#
#         # 将transformer输出和并行bottleneck模块的结果相加
#         combined_output = transformer_output.permute(1, 2, 0).reshape(b, self.c2, w, h) + self.parallel_bottleneck_block(x)
#
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads, num_layers):
        ...
        self.parallel_res_block = ResidualBlock(c2, c2)  # 并行的残差模块
    ...
    def forward(self, x):
        ...
        # 将transformer输出和并行残差模块的结果相加
        combined_output = transformer_output.permute(1, 2, 0).reshape(b, self.c2, w, h) + self.parallel_res_block(x)
...
#         self.c2 = c2
#
#         self.parallel_conv = nn.Conv2d(c2, c2, kernel_size=3, stride=1, padding=1)  # 并行的卷积层
#
#     def forward(self, x):
#         if self.conv is not None:
#             x = self.conv(x)
...
#         transformer_output = self.tr(p + self.linear(p))
#
#         # 将transformer输出和并行卷积层的结果相加
#         combined_output = transformer_output.permute(1, 2, 0).reshape(b, self.c2, w, h) + self.parallel_conv(x)
#
#         return combined_output
...
class PyConv4(nn.Module):
    ...
    def __init__(self, inplans, planes, pyconv_kernels=[3, 5, 7, 9], stride=1, pyconv_groups=[1, 4, 8, 16]):
        ...
        self.conv2_1 = nn.Sequential(
            conv(inplans, planes // 4, kernel_size=pyconv_kernels[0], padding=1,
                 stride=stride, dilation=1, groups=pyconv_groups[0]),
        )
        self.conv2_2 = nn.Sequential(
            conv(inplans, planes // 4, kernel_size=pyconv_kernels[0], padding=1, stride=1, groups=pyconv_groups[1]),
            conv(planes // 4, planes // 4, kernel_size=pyconv_kernels[0], padding=2, stride=stride, dilation=2,
                 groups=pyconv_groups[1]),
        )
        self.conv2_3 = nn.Sequential(
            conv(inplans, planes // 4, kernel_size=pyconv_kernels[0], padding=1, stride=1, dilation=1,
                 groups=pyconv_groups[2]),
            conv(planes // 4, planes // 4, kernel_size=pyconv_kernels[0], padding=2, stride=1, dilation=2,
                 groups=pyconv_groups[2]),
            conv(planes // 4, planes // 4, kernel_size=pyconv_kernels[0], padding=3, stride=stride, dilation=3,
                 groups=pyconv_groups[2]),
        )
        ...
        self.conv2_4 = nn.Sequential(
            ...
            conv(planes // 4, planes // 4, kernel_size=pyconv_kernels[0], padding=2, stride=1, dilation=2,
                 groups=pyconv_groups[3]),
            conv(planes // 4, planes // 4, kernel_size=pyconv_kernels[0], padding=3, stride=1, dilation=3,
                 groups=pyconv_groups[3]),
            conv(planes // 4, planes // 4, kernel_size=pyconv_kernels[0], padding=1, stride=stride, dilation=1,
                 groups=pyconv_groups[3]),
        )
...
class PyConv3(nn.Module):
    ...
    def __init__(self, inplans, planes, pyconv_kernels=[3, 5, 7], stride=1, pyconv_groups=[1, 4, 8]):
        ...
        self.conv2_3 = nn.Sequential(
            conv(inplans, planes // 2, kernel_size=pyconv_kernels[0], padding=1, stride=1, dilation=1,
                 groups=pyconv_groups[2]),
            conv(planes // 2, planes // 2, kernel_size=pyconv_kernels[0], padding=2, stride=1, dilation=2,
                 groups=pyconv_groups[2]),
            conv(planes // 2, planes // 2, kernel_size=pyconv_kernels[0], padding=3, stride=stride, dilation=3,
                 groups=pyconv_groups[2]),
        )
...
class PyConv2(nn.Module):
    ...
    def __init__(self, inplans, planes, pyconv_kernels=[3, 5], stride=1, pyconv_groups=[1, 4]):
        ...
        self.conv2_1 = nn.Sequential(
            conv(inplans, planes // 2, kernel_size=pyconv_kernels[0], padding=1,
                 stride=stride, dilation=1, groups=pyconv_groups[0]),
        )
        ...
        self.conv2_2 = nn.Sequential(
            conv(inplans, planes // 2, kernel_size=pyconv_kernels[0], padding=1, stride=1, dilation=1,
                 groups=pyconv_groups[1]),
            conv(planes // 2, planes // 2, kernel_size=pyconv_kernels[0], padding=2, stride=stride, dilation=2,
                 groups=pyconv_groups[1]),
        )
...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        image_features = F.interpolate(image_features, size=size, mode='nearest')
